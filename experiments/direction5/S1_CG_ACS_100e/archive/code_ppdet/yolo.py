...
class YOLOv3(BaseArch):
    ...
    def __init__(self,
        ...
        Args:
            ...
            aux_cls_head (nn.Layer|None): S1 CG-ACS image-level auxiliary
                classification head. Default None (disabled). Train-only,
        ...
        self.aux_cls_head = aux_cls_head
    ...
    def from_config(cls, cfg, *args, **kwargs):
        ...
        # S1 CG-ACS: optional image-level auxiliary classification head
        aux_cls_head = None
        if 'aux_cls_head' in cfg:
            kwargs = {'input_shape': neck.out_shape}
            aux_cls_head = create(cfg['aux_cls_head'], **kwargs)

        return {
            'backbone': backbone,
            'neck': neck,
            "yolo_head": yolo_head,
            "aux_cls_head": aux_cls_head,
        }
    ...
    def _forward(self):
        ...
        if self.training:
            if self.aux_cls_head is not None and self.inputs.get(
                    'aux_mode', 'det') == 'cls':
                # S1 CG-ACS: PV_CORE classification-only stream.
                # No bbox -> detection head is skipped entirely.
                aux_logits = self.aux_cls_head(neck_feats)
                aux_loss = self.aux_cls_head.loss(
                    aux_logits, self.inputs['aux_cls_target'])
                return {'loss': aux_loss, 'loss_aux_cls': aux_loss}

            yolo_losses = self.yolo_head(neck_feats, self.inputs)

            if self.aux_cls_head is not None:
                # S1 CG-ACS: det stream also receives image-level cls supervision
                # (multi-hot target built from gt boxes present in each image).
                aux_logits = self.aux_cls_head(neck_feats)
                aux_target = self.aux_cls_head.build_target_from_gt(
                    self.inputs['gt_class'], self.aux_cls_head.num_classes)
                aux_loss = self.aux_cls_head.loss(aux_logits, aux_target)
                yolo_losses['loss_aux_cls'] = aux_loss
                yolo_losses['loss'] = yolo_losses['loss'] + aux_loss
